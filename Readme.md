...
We assume that most of these scripts will be run from within your Yocto
workspace; in effect, from the TOPDIR or BUILDDIR directory, and that you've
correctly sourced the oe-init-build-env script.
...
wic_wrsdcard.sh       : (We're assuming the image is to be written to an (u)SD card mmcblk0)
                        It genertaes the WIC image(s) (auto-detecting the image type (eg. core-image-base)
						and the machine value; if sucessful, it attempts to write it out
